...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        // natural number to store mod
        NaturalNumber mod = n.newInstance();
        // natural number constant : 2
        NaturalNumber two = n.newInstance();
        two.setFromInt(2);
        // copy of n for division
        NaturalNumber nCopy = n.newInstance();
        nCopy.copyFrom(n);

        // set mod equal to n/2
        mod.add(nCopy.divide(two));
        return mod.isZero();
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        // p/2
        NaturalNumber halfP = p.newInstance();
        halfP.copyFrom(p);
        halfP.divide(two);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        if (w.compareTo(one) == 0 || wCopy.compareTo(one) != 0) {
            decision = true;
        }
        return decision;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            // if n is even then it can't be prime
            isPrime = false;
        } else {
            int witnessCount = 0;
            NaturalNumber nMinusTwo = n.newInstance();
            nMinusTwo.add(n);
            nMinusTwo.subtract(two);
            nMinusTwo.subtract(two);

            // generate 30 random numbers from (2, n-2)
            //and check to see if they are WitnessToCompositeness
            for (int i = 0; i < 30; i++) {
            ...
            // if a witnessToCompositness was generated then n is not prime
            if (witnessCount > 0) {
                isPrime = false;
            } else {
                isPrime = true;
