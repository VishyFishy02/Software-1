...
public final class RSSAggregator {
            ...
            if (item.child(titlePos).numberOfChildren() > 0) {
                ...
                if (item.child(descPos).numberOfChildren() > 0) {
                    out.print("<td><a href=\""
                            + item.child(linkPos).child(0).label() + "\">");
                    out.print(
                            item.child(descPos).child(0).label() + "</a></td>");

                } else {
                    out.println("<td> No title or description availible </td>");
                }
            }
        } else if (descPos != -1) {
            if (item.child(descPos).numberOfChildren() > 0) {
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        //simpleWriter object to write to an html file
        SimpleWriter write = new SimpleWriter1L(file);
    ...
    public static void main(String[] args) {
        ...
        // get xml file name
        out.print(
                "Enter the name of an XML file containing a list of URLs for RSSv2.0 feeds: ");
        ...
        // print out title of Main feeds page
        XMLTree feeds = new XMLTree1(xmlFile);
        if (feeds.hasAttribute("title")) {
            write.println("<html><head><title>" + feeds.attributeValue("title")
                    + "</title></head><body><h2>"
                    + feeds.attributeValue("title") + "</h2>");
        } else {
            write.println("<html><head><title>" + "No title availible"
                    + "</title></head><body><h2>" + "No title avilible"
                    + "</h2>");
        }

        // print out bulleted list of rss 2.0 pages
        String feedUrl = "No url";
        String feedName = "No name";
        String feedFile = "No file";
        write.println("<ul>");
        int i = 0;
        while (i < feeds.numberOfChildren()) {
            ...
            write.println("<li><a href=\"" + feedFile + "\">" + feedName
                    + "</a></li>");
            i++;
        }
        // print out footer
        write.println("</ul></body></html>");
